...
arraymessage = ['nem fodendo', 'muito mentira', "sim, é verdade", "ta mentindo", "KKKK nao", "pior que é verdade", "até parece", "verdade confia"]
...
while True:
    ...
    if response.data != None:
        for tweet in response.data:
            try:
                print(tweet.text)
                client.create_tweet(in_reply_to_tweet_id=tweet.id, text=arraymessage[random.randrange(0, 8)])
                start_id = tweet.id
